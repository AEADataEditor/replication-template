...
then
   ...
   if grep -q "POTENTIAL PII = 0" "$indir/PII_stata_scan_summary.txt"; then
      ...
      echo "> [REQUIRED] {{ CRITICAL }} Our scanner identified possible PII. Please verify, and if necessary, remove from deposit. If not removing, please explicitly confirm in your response letter that the identified variables are OK to publish without restrictions." >> "$indir/pii-summary.md"
   fi
   echo "" >> "$indir/pii-summary.md"
